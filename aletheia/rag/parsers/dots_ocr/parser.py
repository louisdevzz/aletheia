...
from aletheia.rag.parsers.dots_ocr.model.inference import inference_with_api
from aletheia.rag.parsers.dots_ocr.utils.consts import (
    image_extensions,
    MIN_PIXELS,
    MAX_PIXELS,
)
from aletheia.rag.parsers.dots_ocr.utils.image_utils import (
    get_image_by_fitz_doc,
    fetch_image,
    smart_resize,
)
from aletheia.rag.parsers.dots_ocr.utils.doc_utils import load_images_from_pdf
from aletheia.rag.parsers.dots_ocr.utils.prompts import dict_promptmode_to_prompt
from aletheia.rag.parsers.dots_ocr.utils.layout_utils import (
    post_process_output,
    draw_layout_on_image,
    pre_process_bboxes,
)
from aletheia.rag.parsers.dots_ocr.utils.format_transformer import layoutjson2md
...
class DotsOCRParser:
    ...
    def _inference_with_vllm(self, image, prompt):
        ...
        print(f"    [DotsOCR] 🔌 Calling {self.model_name} API...")
        try:
            ...
            if response:
                print(
                    f"    [DotsOCR] ✅ API success in {elapsed:.1f}s ({len(response)} chars)"
                )
            else:
                print(
                    f"    [DotsOCR] ⚠️  API returned empty response after {elapsed:.1f}s"
                )
            return response
        except Exception as e:
            elapsed = time.time() - start_time
            print(
                f"    [DotsOCR] ❌ API failed after {elapsed:.1f}s: {type(e).__name__}: {e}"
            )
            raise
    ...
    def get_prompt(
        ...
        # Always use default prompt (optimized for Kimi)
    ...
    def _parse_single_image(
        ...
        if prompt_mode in [
            ...
            if (
                ...
                if (
                    ...
                    md_content = layoutjson2md(
                        origin_image, cells, text_key="text", model_name=self.model_name
                    )
            ...
            if md_content is None:
                raise ValueError(
                    f"API returned None for {save_name}. Check API key and model availability."
                )

            md_file_path = os.path.join(save_dir, f"{save_name}.md")
            with open(md_file_path, "w", encoding="utf-8") as md_file:
                md_file.write(md_content)
            result.update({"md_content_path": md_file_path, "content": md_content})
